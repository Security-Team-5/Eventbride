...
function AdminServices() {
        ...
        fetch("api/services/admin", {
            ...
            .then((data) => {
                ...
                const otherServices = Array.isArray(data[0].otherServices)
                    ? data[0].otherServices.map((otherService) => ({ ...otherService, type: "other-services" }))
                    : []
                const venues = Array.isArray(data[0].venues)
                    ? data[0].venues.map((venue) => ({ ...venue, type: "venues" }))
                    : []
    ...
    function updateService(service) {
        // First get the current service to preserve userDTO
        fetch(`/api/${service.type}/${service.id}`, {
            headers: {
                "Content-Type": "application/json",
                Authorization: `Bearer ${jwt}`,
            },
            ...
            .then((currentService) => {
                // Preserve the userDTO from the current service
                const serviceToUpdate = {
                    ...service,
                    userDTO: currentService.userDTO,
                }

                // Now update with the preserved userDTO
                return fetch(`/api/${service.type}/admin/${service.id}`, {
                    headers: {
                        "Content-Type": "application/json",
                        Authorization: `Bearer ${jwt}`,
                    },
    ...
    function deleteService(serviceId, serviceType) {
        fetch(`/api/${serviceType}/admin/${serviceId}`, {
            ...
            .then((response) => {
                ...
                if (response.ok) setServices((prevServices) => prevServices.filter((service) => service.id !== serviceId))
            })
                        ...
                        >
                            <div>
                                ...
                                {editingService?.id === service.id ? (
                                    <div className="service-info">
                                        <form onSubmit={handleSave}>
                                            <div className="form-group">
                                                <label htmlFor="name">
                                                    <strong>Título del Servicio:</strong>
                                                </label>
                                                <input
                                                    type="text"
                                                    name="name"
                                            ...
                                            <div className="form-group">
                                                <label htmlFor="cityAvailable">
                                                    <strong>Ciudad Disponible:</strong>
                                                </label>
                                                <input
                                                    type="text"
                                                    name="cityAvailable"
                                            ...
                                            <div className="form-group">
                                                <label htmlFor="limitedByPricePerGuest">
                                                    <strong>Limitado por Precio por Invitado:</strong>
                                                </label>
                                                <select
                                                    name="limitedByPricePerGuest"
                                                    value={editingService.limitedByPricePerGuest}
                                                    onChange={handleInputChange}
                                                >
                                                    <option value={true}>Sí</option>
                                                    <option value={false}>No</option>
                                            ...
                                            <div className="form-group">
                                                <label htmlFor="limitedByPricePerHour">
                                                    <strong>Limitado por Precio por Hora:</strong>
                                                </label>
                                                <select
                                                    name="limitedByPricePerHour"
                                                    value={editingService.limitedByPricePerHour}
                                                    onChange={handleInputChange}
                                                >
                                                    <option value={true}>Sí</option>
                                                    <option value={false}>No</option>
                                            ...
                                            <div className="form-group">
                                                <label htmlFor="picture">
                                                    <strong>Enlace de la Imagen:</strong>
                                                </label>
                                                <input
                                                    style={{ fontSize: "16px" }}
                                                    type="text"
                                                    name="picture"
                                                    value={editingService.picture}
                                                    onChange={handleInputChange}
                                                />
                                            </div>
                                            {service.type === "venues" && (
                                            ...
                                            <img
                                                src={service.picture || "/placeholder.svg"}
                                                alt={service.name}
                                    ...
                                    <div className="service-info">
                                        <p>
                                            <strong>Disponible:</strong> {service.available ? "Sí" : "No"}
                                        </p>
                                        <p>
                                            <strong>Ciudad Disponible:</strong> {service.cityAvailable}
                                        </p>
                                        <p>
                                            <strong>Precio por Invitado:</strong>{" "}
                                        ...
                                        {service.type === "venues" && (
                                            <>
                                                <p>
                                                    <strong>Código Postal:</strong> {service.postalCode}
                                                </p>
                                                <p>
                                                    <strong>Coordenadas:</strong> {service.coordinates}
                                                </p>
                                                <p>
                                                    <strong>Dirección:</strong> {service.address}
                                                </p>
                                                <p>
                                                    <strong>Capacidad Máxima:</strong> {service.maxGuests}
                                                </p>
                                                <p>
                                                    <strong>Superficie:</strong> {service.surface} m²
                                                </p>
                                                <p>
                                                    <strong>Hora de apertura:</strong> {service.earliestTime}
                                                </p>
                                                <p>
                                                    <strong>Hora de cierre:</strong> {service.latestTime}
                                                </p>
                                            </>
                                        )}
                                        {service.type === "other-services" && (
                                            <>
                                                <p>
                                                    <strong>Tipo de Servicio:</strong> {service.otherServiceType}
                                                </p>
                                                <p>
                                                    <strong>Información Extra:</strong> {service.extraInformation}
                                                </p>
                                            </>
                                        )}
                                    </div>
